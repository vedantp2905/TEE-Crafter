...
|-----------|-------------|-----------|
| SIEM in-TEE refusal | [`siem_health.assert_siem_healthy`](../apps/cli/src/tee_crafter/templates/common/siem_health.py) | Every attested request routes through `fail_closed_wrap` on the platform RA-TLS server (`app.template.py` per platform). When the SIEM health file is stale, missing, or carries a non-`pass` last export, the gate returns `{"error":"siem_blackout"}` instead of touching user code. **Armed on the 8 CVM platforms only.** `nitro-aws` / `sgx-azure` wire the wrapper but run the exporter host-side and pass no SIEM env into the TEE, so `is_fail_closed` is `False` and the wrapper is a pass-through — export is detective, not preventive. Enforced as `siem_sidecar.PREVENTIVE_GATE_PLATFORMS`. |

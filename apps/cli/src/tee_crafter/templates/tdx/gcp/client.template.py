...
def verify_qe_report_binding(quote_bytes: bytes) -> bool:
    ...
    server-supplied blob came up short, and the caller downgraded that to an
    informational line — so a 764-byte quote signed with the sender's own P-256
    key passed every check.  Length checks on untrusted input can only ever
    tell you the input is unusable, never that it is acceptable.
    """
    signed_data_len = 632
    sig_offset = signed_data_len + 4
...
def verify_pck_cert_chain(quote_bytes: bytes) -> dict:
    """Verify the PCK certificate chain embedded in the TDX DCAP quote against Intel's Root CA.

    Returns ``{"ok": True, "pck_leaf": <x509.Certificate>}`` when the chain
    validates to the pinned root, and ``{"ok": False, "reason": "..."}``
    otherwise.  As with ``verify_qe_report_binding``, a quote with no cert data
    is a failure rather than an "absent" free pass.  The leaf is returned
    because its key signs the QE report — see ``verify_qe_report_signature``.
    """
    from cryptography import x509
    from cryptography.hazmat.backends import default_backend
...
# this commitment".
#
# These four must stay byte-identical to tdx/gcp/app.template.py.  The label
...
# binding (the RA-TLS certificate's embedded quote, tdx-gcp) from theirs and
...
def _attest_binding_preimage(*fields: bytes) -> bytes:
    ...
    identical to ``_attest_binding_preimage`` in tdx/gcp/app.template.py.
    """
...
def verify_ratls_connection(host: str, port: int, ratls_nonce: bytes | None = None) -> tuple:
    """Connect to the TDX VM via TLS and verify the RA-TLS attestation (DCAP only for GCP).

    ``ratls_nonce`` is round-tripped into ``ATTESTATION_REPORT`` for audit correlation.
    """
    ...
    return _verify_dcap_attestation(
        conn, quote_bytes,
        cert_der=cert_der,
        server_cd=server_cd,
        ratls_nonce=ratls_nonce,
    )
...
def _verify_dcap_attestation(
    conn,
    quote_bytes: bytes,
    *,
    cert_der: bytes,
    server_cd: str | None = None,
    ratls_nonce: bytes | None = None,
) -> tuple:
    """Verify a standard TDX DCAP quote (GCP uses standard DCAP, no Azure HCLA)."""
    ...
    # as in tdx/azure.  It read TEE_TCB_SVN[0] as the module's major SVN and
    # [1] as the minor; Intel's Quote Verification Library has that the other
    # way round (TDX_MODULE_MAJOR_SVN_INDEX = 1), so the check passed a real
    # module 1.2 and refused a real module 3.0.  The module is now evaluated
    # against signed Intel collateral in enforce_platform_tcb_status below:
    # MRSIGNERSEAM and SEAMATTRIBUTES matched against tdxModuleIdentities, and
    # the module's own tcbStatus converged with the platform status.

    # QE identity is no longer checked here: it needs the PCK chain (for the
    # FMSPC and the CRLs), so it now runs alongside the platform TCB evaluation
    # after the chain walk.  See enforce_platform_tcb_status below.
    ...
    # M-06: no self-pinning — an unpinned client fails closed unless the
    # operator opts out.
    ...
    if not verify_qe_report_binding(quote_bytes):
        conn.close()
        print("FATAL: QE report binding verification FAILED — attestation key authenticity cannot be confirmed.",
              file=sys.stderr)
        sys.exit(1)
    ...
    if not pck_result.get("ok"):
        conn.close()
        print("FATAL: PCK certificate chain verification FAILED — cannot verify hardware root of trust.",
              file=sys.stderr)
        sys.exit(1)
    print("  PCK certificate chain: PASSED", file=sys.stderr)

    # Verify the QE report is signed by the PCK leaf key.  The signature and
    # binding checks above both read the attestation key out of the quote, so
    # they prove nothing on their own; this step is what makes them meaningful.
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 client_tdx_gcp.py <host_ip> [port]", file=sys.stderr)
        sys.exit(1)
    ...
    print(f"Connecting to TDX confidential VM at {host_ip}:{port} via RA-TLS (GCP)...", file=sys.stderr)
    try:
        conn, quote_info = verify_ratls_connection(host_ip, port, ratls_nonce=ratls_nonce)
        print("TDX RA-TLS Attestation Verification Passed! (GCP)", file=sys.stderr)
    except Exception as e:

...
variable "instance_type" {
  ...
  description = "EC2 instance type. Must be M6a, C6a, or R6a family for AMD SEV-SNP."
}
...
#
# These variables are auto-set by the CLI when ``--siem-*`` flags are used
# (see tee_crafter/cli/commands/deploy/siem_egress_terraform.py).  Defaults
# are no-ops, so existing deployments are unaffected.

variable "siem_provision_logs_endpoint" {
  type        = bool
  default     = false
  description = "When true, create a com.amazonaws.<region>.logs Interface VPC Endpoint so the in-TEE SIEM exporter can ship to CloudWatch Logs without leaving the VPC. Set automatically by --siem cloudwatch + --siem-egress private/auto."
}
...
variable "siem_cloudwatch_log_group" {
  type        = string
  default     = ""
  description = "When non-empty AND --siem cloudwatch is selected, attach a least-privilege IAM policy letting the host role write SIEM events to this CloudWatch log group (logs:CreateLogStream + logs:PutLogEvents)."
}

# BYOK on AWS KMS for SNP-AWS workloads.  See nitro/main.template.tf
# for the canonical doc string.  Empty (default) leaves IAM untouched.
variable "byok_aws_kms_arn" {
  type        = string
  default     = ""
  description = "Optional customer KMS key ARN to grant the SNP-AWS instance role kms:Decrypt on (for --byok aws-kms boot-time release)."
}

# UEFI Secure Boot — for snp-aws this is an *AMI property* (the UEFI NVRAM
# is captured into Image.UefiData at bake time), not a runtime knob.  This
# variable serves three purposes:
#   1) Cross-checks at plan time that the operator's intent matches the
#      AMI being launched (via the `precondition` block on the instance).
#   2) Surfaces in `terraform output secure_boot_mode` so the CLI's deploy
#      summary and any CI wrapper can display the current posture.  (There is
#      no TEE-Crafter GUI; `apps/` ships the CLI only.)
#   3) Forces the base-AMI data source to require `uefi-preferred` boot mode
#      when set to true (defense-in-depth against a legacy-BIOS Ubuntu AMI
#      regression).
# Defaults to TRUE since 2026 — Secure Boot is the assumed posture for every
# non-GPU TEE-Crafter platform.  The CLI deploy validators auto-set this from
# the AMI tag, and the bake CLI defaults to enrolling SB keys.  To deploy a
# non-SB AMI (e.g., a legacy unbaked dev workflow), set
# TF_VAR_enable_secure_boot=false and use TEE_CRAFTER_ALLOW_UNBAKED_BASE_AMI=1.
variable "enable_secure_boot" {
  type        = bool
  default     = true
  description = "Assert that the AMI being launched has UEFI Secure Boot keys pre-enrolled (bake with `bake-ami --enable-secure-boot`; on by default). When true, the instance precondition fails fast unless the AMI carries the `tee-crafter-secure-boot=enabled` tag. Set to false explicitly only for legacy unbaked dev AMIs."
}
...
resource "aws_vpc" "deployment" {
  ...
  tags = {
    Name         = "tee-crafter-snp-vpc-${local.did}"
    Project      = "tee-crafter"
...
resource "aws_subnet" "private" {
  ...
  tags = {
    Name    = "tee-crafter-snp-private-subnet"
    Project = "tee-crafter"
  }
}
...
resource "aws_route_table" "private" {
  ...
  tags = {
    Name    = "tee-crafter-snp-private-rt"
    Project = "tee-crafter"
  }
}
...
resource "aws_subnet" "public" {
  ...
  tags = {
    Name    = "tee-crafter-snp-public-subnet"
    Project = "tee-crafter"
  }
}
...
resource "aws_route_table" "public" {
  ...
  tags = {
    Name    = "tee-crafter-snp-public-rt"
    Project = "tee-crafter"
  }
}
...
resource "aws_cloudwatch_log_group" "vpc_flow_logs" {
  name              = "/tee-crafter/snp-vpc-flow-logs/${random_id.bucket_suffix.hex}"
...
resource "aws_iam_role" "flow_log_role" {
  name_prefix = "tee-crafter-snp-flow-log-"
...
resource "aws_flow_log" "vpc" {
  ...
  tags = {
    Name    = "tee-crafter-snp-vpc-flow-log"
    Project = "tee-crafter"
  }
}
...
# Ubuntu 22.04 LTS x86_64 (SEV-SNP compatible)
data "aws_ami" "ubuntu" {
  most_recent = true
  ...
  # When SB is asserted, refuse legacy-BIOS-only Ubuntu AMIs.  Canonical
  # has shipped uefi-preferred AMIs since 2023, so this filter is a
  # no-op for current images and a guardrail against future regressions.
  dynamic "filter" {
...
# When `enable_secure_boot = true`, look up the operator-provided custom
# AMI's tags so the instance preconditions can confirm it was actually
# baked with the `tee-crafter-secure-boot=enabled` tag from
# `bake-ami --enable-secure-boot`.
...
resource "aws_kms_key" "bucket_key" {
  count                   = local.create_bucket ? 1 : 0
  description             = "SSE-KMS key for TEE-Crafter SNP deployment bucket"
...
resource "aws_security_group" "snp_sg" {
  count       = local.create_sg ? 1 : 0
  name_prefix = "tee-crafter-snp-sg-"
  description = "SEV-SNP VM: no ingress, egress for VPC endpoints and setup"
  vpc_id      = aws_vpc.deployment.id

  # Default 443/tcp egress.  When `siem_egress_cidrs` is non-empty the
  # default 0.0.0.0/0 rule (used during package installs / public SIEM
  # ingest) is *replaced* by per-CIDR rules below, so SIEM traffic is
  # locked to the configured collector list.
  dynamic "egress" {
    ...
    content {
      ...
      description = var.allow_setup_egress ? "HTTPS for VPC endpoints + package repos" : "HTTPS restricted to VPC (custom AMI mode)"
    }
  }
  ...
  dynamic "egress" {
    ...
    content {
      ...
      description = "HTTP for package repos during setup"
    }
  }
  ...
  egress {
    ...
    protocol    = "udp"
    cidr_blocks = [aws_vpc.deployment.cidr_block]
    description = "DNS restricted to VPC resolver"
  }
  egress {
    from_port   = 53
    to_port     = 53
...
locals {
  ...
  instance_profile_name  = local.create_iam ? aws_iam_instance_profile.snp_profile[0].name : var.existing_instance_profile_name
  security_group_id      = local.create_sg ? aws_security_group.snp_sg[0].id : var.existing_security_group_id
  enclave_role_arn       = local.create_iam ? aws_iam_role.snp_role[0].arn : var.existing_enclave_role_arn
...
# release hangs and the workload never starts (fail-closed).  Private DNS makes
# kms.<region>.amazonaws.com resolve to the endpoint ENI.
resource "aws_vpc_endpoint" "kms" {
  ...
  tags = {
    Name    = "tee-crafter-snp-byok-kms-vpce"
    Project = "tee-crafter"
  }
}
...
#
# Created on demand when the operator selects ``--siem cloudwatch``
# (the CLI flips ``siem_provision_logs_endpoint`` to true).  Routes
# CloudWatch Logs traffic privately so the deployment doesn't need a
# NAT gateway for SIEM at all.
...
resource "aws_vpc_endpoint" "siem_logs" {
  ...
  tags = {
    Name    = "tee-crafter-snp-siem-logs-vpce"
    Project = "tee-crafter"
...
resource "aws_iam_role" "snp_role" {
  count       = local.create_iam ? 1 : 0
  name_prefix = "tee-crafter-snp-role-"
...
resource "aws_iam_role_policy_attachment" "ssm_core" {
  count      = local.create_iam ? 1 : 0
  role       = aws_iam_role.snp_role[0].name
  policy_arn = "arn:aws:iam::aws:policy/AmazonSSMManagedInstanceCore"
}

resource "aws_iam_role_policy" "snp_policy" {
  count       = local.create_iam ? 1 : 0
  name_prefix = "tee-crafter-snp-policy-"
  role        = aws_iam_role.snp_role[0].id
...
resource "aws_iam_instance_profile" "snp_profile" {
  count       = local.create_iam ? 1 : 0
  name_prefix = "tee-crafter-snp-profile-"
  role        = aws_iam_role.snp_role[0].name
}

# Least-privilege CloudWatch Logs PutEvents grant for the SIEM exporter
# running inside the TEE.  Only created when --siem cloudwatch was passed
# AND we're managing the host IAM role.
resource "aws_iam_role_policy" "snp_siem_cloudwatch_logs" {
  count       = local.create_iam && var.siem_cloudwatch_log_group != "" ? 1 : 0
  name_prefix = "tee-crafter-snp-siem-logs-"
  role        = aws_iam_role.snp_role[0].id
...
# is non-empty.
resource "aws_iam_role_policy" "snp_byok_kms_decrypt" {
  count       = local.create_iam && var.byok_aws_kms_arn != "" ? 1 : 0
  name_prefix = "tee-crafter-snp-byok-decrypt-"
  role        = aws_iam_role.snp_role[0].id
...
# --- EC2 Instance (AMD SEV-SNP, private-only, no SSH) ---

resource "aws_instance" "snp_host_spot" {
  ...
  # AMD SEV-SNP enablement — can only be set at launch time
  cpu_options {
    amd_sev_snp = "enabled"
  }
  ...
  lifecycle {
    ...
    # actually baked with `bake-ami --enable-secure-boot`.  The bake step
    # tags successful SB AMIs with `tee-crafter-secure-boot=enabled`; if
    # that tag is missing — or no custom AMI was supplied at all (stock
    # Canonical Ubuntu has no pre-enrolled SB keys) — we abort here rather
    # than producing a deployment whose attestation posture silently doesn't
    # match the operator's intent.  Set TF_VAR_enable_secure_boot=false to
    # opt out (legacy unbaked dev workflow only).
    precondition {
      ...
      error_message = "enable_secure_boot=true (the default since 2026) requires a custom AMI baked with `tee-crafter internal bake-ami --tee-platform snp-aws` carrying tag tee-crafter-secure-boot=enabled. Stock Canonical AMIs are not Secure-Boot-enrolled. Either re-bake with --enable-secure-boot (default), or pass TF_VAR_enable_secure_boot=false to opt out (dev only)."
    }
  }
  ...
  tags = {
    Name         = "TEECrafterSNPHost-Spot-${local.did}"
    Project      = "tee-crafter"
    DeploymentId = local.did
  }
}

resource "aws_instance" "snp_host_ondemand" {
  ...
  tags = {
    Name         = "TEECrafterSNPHost-OnDemand-${local.did}"
    Project      = "tee-crafter"
    DeploymentId = local.did
  }
}
...
output "instance_id" {
  value = var.use_spot_instance ? aws_instance.snp_host_spot[0].id : aws_instance.snp_host_ondemand[0].id
}

output "private_ip" {
  value = var.use_spot_instance ? aws_instance.snp_host_spot[0].private_ip : aws_instance.snp_host_ondemand[0].private_ip
}
...
output "setup_egress_mode" {
  ...
  description = "NET-1: post-bake egress lockdown state. open-for-setup means a NAT gateway is attached for first-boot package installs; locked-down means only intra-VPC / attestation egress remains."
}
...
output "secure_boot_mode" {
  ...
  description = "UEFI Secure Boot posture for this deployment. enforcing → AMI was baked with `bake-ami --enable-secure-boot` and instances boot with kernel lockdown=integrity. permissive → instances boot UEFI but firmware is in Setup Mode."
}

# DEP-005: structured evidence that the launched instance has IMDSv2
# *required* (http_tokens = "required") rather than just opt-in.  The
# orchestrator reads this output and emits the DEP-005 ledger row at
# PASS / FAIL instead of WARN-on-missing.  Hard-coded ``true`` because
# both ``aws_instance`` blocks above set ``http_tokens = "required"``
# unconditionally; if a future edit downgrades to ``optional`` this

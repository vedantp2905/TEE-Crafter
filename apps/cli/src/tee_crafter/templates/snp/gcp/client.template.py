...
# Injected at build time.  If both Milan and Genoa chains are baked in,
# the client tries each at runtime and accepts whichever validates the
# endorsement cert.  Placeholder tokens are intentionally kept out of this
# comment block; the template renderer replaces text globally and a multi-
# line PEM would otherwise bleed into non-comment lines.
...
def extract_snp_evidence_from_cert(cert_der: bytes) -> tuple:
    """Extract the SNP attestation report and endorsement cert from a TLS certificate."""
...
def verify_snp_report_signature(report: bytes, endorsement_pem: bytes) -> bool:
    """Verify the ECDSA-384 signature on the SNP attestation report using the VCEK."""
...
def verify_endorsement_cert_chain(endorsement_pem: bytes):
    ...
    chain (SNP-2 cert-chain auto-selection by CHIP_ID signature).
...
def verify_ratls_connection(host: str, port: int, ratls_nonce: bytes | None = None) -> tuple:
    """Connect to the SNP VM via TLS and verify the attestation evidence.

    ``ratls_nonce`` is *not* consumed here: the report carried in the TLS
    certificate is minted at certificate-rotation time and cannot echo a
    per-connection challenge.  Freshness and channel binding are established
    afterwards in ``main()`` via ``verify_live_challenge``, which sends this
    nonce to the server and checks the report it signs in reply.  The value
    is round-tripped into the ``ATTESTATION_REPORT`` line for audit
    correlation.
    """
    ...
    # Validate sig_algo == 1 (ECDSA P-384 with SHA-384; AMD SEV-SNP ABI §7.3)
    ...
    # The label of the chain that validated is the processor family, used
    # below to pick the AMD-SB-3015 SNP firmware SVN floor.
    ...
    # Without a pinned measurement the chain above proves only "some genuine
    # SEV-SNP guest" — not which workload is running inside it — so an
    # unpinned client fails closed.
    allow = _measurement_allowlist()
    if allow:
    ...
    # The legacy "default" chain carries no family signal, so only the two
    ...
    # COMMITTED_TCB <= REPORTED_TCB anti-rollback (AMD SEV-SNP ABI §4.4)
    ...
    # for the deploy orchestrator (see attestation_report.py).
    try:
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 client_snp_gcp.py <host_ip> [port]", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        print("AMD SEV-SNP RA-TLS Attestation Verification Passed! (GCP)", file=sys.stderr)
    except Exception as e:
    ...
    # M-02: send the client nonce we generated above (previously a second,

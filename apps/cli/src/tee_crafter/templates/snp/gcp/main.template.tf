...
variable "machine_type" {
  ...
  # N2D only. C2D and C3D offer AMD SEV, which is not SEV-SNP: no integrity
  # protection and no attestation report, so the evidence chain this project is
  # built on does not exist there. The CLI catalog rejects them for this
  # platform; see docs/watchlist.md.
  description = "GCP machine type. Must be N2D for AMD SEV-SNP."
}

variable "min_cpu_platform" {
  type        = string
  default     = "AMD Milan"
  description = "Host CPU generation to require, matching the generation the bake captured its launch measurement on. Empty string lets Google choose."
}
...
# Set to true by the CLI (export_byok_tf_vars) when --byok gcp-kms is used.
# The in-TEE secret bootstrap calls Cloud KMS to release the DEK / unseal the
# .env at boot; egress is deny-all except the restricted Google APIs VIP
# (199.36.153.8/30), so we publish a private Cloud DNS zone that resolves
# *.googleapis.com to that VIP.  Without it, cloudkms.googleapis.com resolves
# to a public IP that the firewall drops and the release hangs (fail-closed).
...
resource "local_sensitive_file" "ssh_private_key" {
  ...
  filename        = abspath("${path.module}/snp_gcp_ssh_key")
...
resource "google_compute_subnetwork" "subnet" {
  name          = "tee-crafter-snp-subnet-${local.did}"
...
# Routes *.googleapis.com to the restricted VIP (199.36.153.8/30, already the
# only allowed HTTPS egress) so the in-TEE Cloud KMS decrypt resolves to a
# reachable private address under deny-all egress.  Only created for
# --byok gcp-kms; the metadata resolver (169.254.169.254) serves this zone.
resource "google_dns_managed_zone" "googleapis" {
  count       = var.byok_gcp_kms ? 1 : 0
  name        = "tee-crafter-snp-googleapis-${local.did}"
...
resource "google_compute_firewall" "allow_iap_ssh" {
  name    = "tee-crafter-snp-allow-iap-ssh-${local.did}"
...
resource "google_compute_firewall" "deny_all_ingress" {
  name    = "tee-crafter-snp-deny-ingress-${local.did}"
  network = google_compute_network.vpc.name
...
resource "google_compute_firewall" "allow_egress_internal" {
  name      = "tee-crafter-snp-egress-internal-${local.did}"
  network   = google_compute_network.vpc.name
  direction = "EGRESS"
...
resource "google_compute_firewall" "allow_egress_metadata" {
  name      = "tee-crafter-snp-egress-metadata-${local.did}"
  network   = google_compute_network.vpc.name
  direction = "EGRESS"
...
resource "google_compute_firewall" "allow_egress_dns" {
  name      = "tee-crafter-snp-egress-dns-${local.did}"
  network   = google_compute_network.vpc.name
  direction = "EGRESS"
...
resource "google_compute_firewall" "allow_egress_setup" {
  ...
  name      = "tee-crafter-snp-egress-setup-${local.did}"
  network   = google_compute_network.vpc.name
  direction = "EGRESS"
...
resource "google_compute_router" "nat_router" {
  ...
  name    = "tee-crafter-snp-router-${local.did}"
  region  = var.gcp_region
  network = google_compute_network.vpc.id
}
...
resource "google_compute_firewall" "allow_egress_siem" {
  ...
  name      = "tee-crafter-snp-egress-siem-${local.did}"
  network   = google_compute_network.vpc.name
  direction = "EGRESS"
...
resource "google_compute_firewall" "deny_all_egress" {
  name      = "tee-crafter-snp-deny-egress-${local.did}"
  network   = google_compute_network.vpc.name
  direction = "EGRESS"
...
locals {
  ...
  tag = "tee-crafter-snp-${local.did}"
}

resource "google_kms_key_ring" "tee_crafter" {
  name     = "tee-crafter-snp-kr-${random_id.bucket_suffix.hex}"
  location = var.gcp_region
}

resource "google_kms_crypto_key" "bucket_key" {
  name            = "tee-crafter-snp-key-${random_id.bucket_suffix.hex}"
...
resource "google_kms_crypto_key" "disk_key" {
  name            = "tee-crafter-snp-disk-key-${random_id.bucket_suffix.hex}"
  key_ring        = google_kms_key_ring.tee_crafter.id
...
resource "google_storage_bucket" "deployment" {
  name     = "tee-crafter-snp-${random_id.bucket_suffix.hex}"
  location = var.gcp_region
...
resource "google_service_account" "vm_sa" {
  account_id   = "tc-snp-${local.did}"
  display_name = "TEE-Crafter SNP VM Service Account"
}
...
resource "google_compute_instance" "snp_vm" {
  name                = "tee-crafter-snp-vm-${local.did}"
  machine_type        = var.machine_type
  zone                = var.gcp_zone
  deletion_protection = false

  # The AMD SEV-SNP launch measurement folds in host firmware and microcode, so
  # it is specific to the host CPU generation. The bake pins the generation it
  # measures on (`--min-cpu-platform "AMD Milan"`, baking/gcp.py) and this asks
  # for the same one, so the digest the deploy attests against is the digest that
  # was captured. Without it the deploy relied on N2D happening to offer nothing
  # newer that SEV-SNP runs on — true today, and not a thing to depend on
  # silently. Set to "" to let Google choose.
...
output "instance_name" {
  value = google_compute_instance.snp_vm.name
}

output "instance_zone" {
  value = google_compute_instance.snp_vm.zone
}

output "vm_private_ip" {
  value = google_compute_instance.snp_vm.network_interface[0].network_ip
}
...
output "setup_egress_mode" {
  ...
  description = "NET-1: post-bake egress lockdown state. open-for-setup provisions a Cloud Router + Cloud NAT for first-boot package installs; locked-down keeps only intra-VPC + Google metadata + attestation egress."
}

output "secure_boot_mode" {
  value       = "enforcing (GCP Shielded VM — enable_secure_boot = true)"
  description = "UEFI Secure Boot posture for this GCP SNP deployment. Always enforcing — google_compute_instance.shielded_instance_config hard-codes enable_secure_boot = true plus vTPM + integrity monitoring."
}

"""GCP VM management via IAP (Identity-Aware Proxy) – SSH/SCP operations.

Replaces direct SSH with IAP tunnels for zero-public-IP secure access.
"""
import logging
import os
import subprocess
...
from tee_crafter.core.remote.gcp_ssh_tunnel import (  # noqa: F401 – re-exports
    IAPTunnel, SSHPortForward,
)
...
# See docs/optimizations.md §3.  Multiplex every ssh/scp call onto a single
# established SSH channel over the IAP local listener so we only pay the
# KEX+auth handshake cost once per VM, not per call.  Disabled via
# TEE_CRAFTER_SSH_MUX=0 for debugging.

# Shared with azure_ssh — see `core.remote.ssh_mux` for why the ControlPath
# needs a length check rather than being composed straight from $TMPDIR.
...
def close_ssh_mux(host: str = "localhost", port: int = 22, user: str = "tee_admin",
                  ssh_private_key_path: str | None = None) -> None:
    """Tear down the multiplexed SSH master for *host:port:user* before
    the IAP tunnel goes away.  Best-effort — silent if no master exists.
    """
    if not _mux_enabled():
        return
    try:
...
# Substrings (case-insensitive) that mark an SSH-layer / transport-layer failure
# we should retry. These never appear in the stderr of a *remote* command that
# legitimately failed — they only come from the local ssh/scp client when the
# server (or the IAP local listener) reset / refused the connection during the
# pre-auth handshake. The most common offender on automation pipelines is
# sshd's MaxStartups limiter or the GCP IAP local listener flaking under
# rapid back-to-back sessions, which surfaces as
...
def _ssh_retry_config() -> Tuple[int, float]:
    """Read retry knobs from env (clamped to sane bounds)."""
    try:
...
def _run_with_ssh_retry(argv: list, *, timeout: int, op_label: str = "SSH"):
    ...
    Returns either a :class:`subprocess.CompletedProcess` (success or
    non-transient failure) or a fabricated CompletedProcess if every attempt
    timed out / hit a transient error. Callers translate this into their own
    return shape.
    """
...
def wait_for_ssh(ssh_private_key_path: str, user: str = "tee_admin",
                 timeout: int = 300, *, host: str = "localhost", port: int = 22) -> bool:
    """Wait until SSH is reachable on host:<port>.

    Exponential-backoff poll (2 s → 10 s cap) so we discover SSH-ready
    VMs ~7-9 s earlier on the fast path while still backing off when
    the VM is genuinely still booting.  See docs/optimizations.md §5.
    """
    ...
    while time.time() - start_t < timeout:
        ...
        try:
            ...
            if result.returncode == 0 and "ok" in result.stdout:
                return True
            err_snippet = (result.stderr or result.stdout or "").strip()[:200]
    ...
    logger.warning("[SSH] gave up after %d attempts (%ds). Last error: %s",
                   attempt, int(time.time() - start_t), last_err)
    return False


def run_ssh_command(command: str, ssh_private_key_path: str, user: str = "tee_admin",
    ...
    Transient SSH transport errors (e.g. ``kex_exchange_identification: read:
    Connection reset by peer`` from sshd's MaxStartups gate or a flaky IAP
    local listener) are retried with bounded exponential backoff. Genuine
    remote-command failures are returned as-is on the first attempt.
    """
    if DEBUG:
...
def upload_file_via_scp(local_path: str, remote_path: str, ssh_private_key_path: str,
                        user: str = "tee_admin", timeout: int = 120,
...
def upload_directory_via_scp(local_dir: str, remote_dir: str, ssh_private_key_path: str,
                             user: str = "tee_admin", timeout: int = 300,
                             *, host: str = "localhost", port: int = 22) -> Tuple[bool, str]:
...
def download_file_via_scp(remote_path: str, local_path: str, ssh_private_key_path: str,
                          user: str = "tee_admin", timeout: int = 600,
                          *, host: str = "localhost", port: int = 22) -> Tuple[bool, str]:
...
def download_directory_via_scp(remote_dir: str, local_dir: str, ssh_private_key_path: str,
                               user: str = "tee_admin", timeout: int = 900,
                               *, host: str = "localhost", port: int = 22) -> Tuple[bool, str]:

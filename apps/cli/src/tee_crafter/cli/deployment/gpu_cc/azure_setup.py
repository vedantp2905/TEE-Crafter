"""First-boot setup for GPU CC VMs on Azure via Bastion tunnel.

Reuses the canonical bake script (setup_gpu_cc_azure.sh) as the single
...
def run_ssh_cloudinit_gpu_cc_azure_setup(
    progress,
    console,
    ssh_key_path: str,
    build_dir: str,
    ...
    """Wait for SSH and perform GPU CC setup on an Azure NCC H100 v5 VM."""
    ...
    if not ok:
        progress.update(t, description="[bold red]✗ SSH timed out via Bastion.[/bold red]")
        return False
    progress.update(t, description="[green]✓ SSH online via Bastion.[/green]")
    ...
    t = progress.add_task("[yellow]Running GPU CC Azure setup script...[/yellow]", total=None)

    from tee_crafter.cli.loaders import load_gpu_cc_azure_setup_template
    script_body = load_gpu_cc_azure_setup_template()
    ...
    scp_ok, scp_msg = upload_file_via_scp(
        tmp.name, "/tmp/setup_gpu_cc_azure.sh",
    ...
    ok_cmd, _out, _err = run_ssh_command(
        "chmod +x /tmp/setup_gpu_cc_azure.sh && sudo /tmp/setup_gpu_cc_azure.sh",
        ssh_key_path, user=admin_user, port=tunnel_port, timeout=7200,
    )
    if not ok_cmd:
        progress.update(t, description="[red]✗ GPU CC Azure setup script failed.[/red]")
        if audit:
    ...
    if audit:
        ...
        audit.record("Phase 4: GPU Setup", "Dual attestation configured (CPU+GPU)", "pass",
                     cpu_tee_type="AMD SEV-SNP", gpu_tee_type="NVIDIA CC",
                     cpu_attestation_ok=True, gpu_attestation_ok=True,
                     combined_valid=True, encrypted_pcie=True)
        audit.record("Phase 5: Post-Deploy", "GPU CC Azure VM setup", "pass",

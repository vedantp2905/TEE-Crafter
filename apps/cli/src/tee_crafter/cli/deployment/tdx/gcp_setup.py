"""Cloud-init / first-boot setup for Intel TDX VMs on GCP via IAP tunnel."""
...
def run_ssh_cloudinit_tdx_gcp_setup(
    progress,
    console,
    ssh_key_path: str,
    build_dir: str,
    ...
    """Wait for SSH and perform first-boot setup on a GCP TDX Confidential VM."""
    ...
    run_ssh_command(
        ...
        "python3-venv python3-dev build-essential curl pkg-config libssl-dev",
    ...
    run_ssh_command(
        "id -u tee_enclave &>/dev/null || sudo useradd -r -m -d /opt/tee-crafter-tdx -s /usr/sbin/nologin tee_enclave; "
        "sudo mkdir -p /opt/tee-crafter-tdx/{app,wheels}; "
        "sudo chown -R tee_enclave:tee_enclave /opt/tee-crafter-tdx",
        ssh_key_path, user=admin_user, port=tunnel_port, timeout=30,
    )
    ...
    run_ssh_command(
        "sudo -u tee_enclave python3 -m venv /opt/tee-crafter-tdx/venv && "
        "/opt/tee-crafter-tdx/venv/bin/pip install --upgrade pip setuptools wheel 2>&1 | tail -1",
    ...
    run_ssh_command(
        ...
        "/opt/tee-crafter-tdx/venv/bin/pip install --no-cache-dir --no-index "
    ...
    import base64
    unit = load_unit("tdx-gcp")
    unit_b64 = base64.b64encode(unit.encode()).decode()
    run_ssh_command(
        f"echo '{unit_b64}' | base64 -d | sudo tee /etc/systemd/system/tee-crafter-tdx.service > /dev/null && "
        "sudo systemctl daemon-reload && sudo systemctl enable tee-crafter-tdx.service",
        ssh_key_path, user=admin_user, port=tunnel_port, timeout=30,
    )
    progress.update(t, description="[green]✓ Systemd service created.[/green]")

    if audit:
        audit.record("Phase 5: Post-Deploy", "TDX GCP VM cloud-init setup", "pass")
